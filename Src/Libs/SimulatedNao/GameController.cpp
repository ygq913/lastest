  ...
  theTeamMessageChannel(new TeamMessageChannel[2] { {inTeamMessage, outTeamMessage}, {inTeamMessage, outTeamMessage} })
{
  ...
  // Initialize external GC sockets
#ifdef WINDOWS
  gcReceiveSocket = INVALID_SOCKET;
  gcSendSocket = INVALID_SOCKET;
#else
  gcReceiveSocket = -1;
  gcSendSocket = -1;
#endif
  memset(&gcSendAddr, 0, sizeof(gcSendAddr));
}

GameController::~GameController()
{
  enableExternalGameController(false);
...
void GameController::enableExternalGameController(bool enable)
{
  if(enable == externalGCEnabled)
    return;
  ...
  if(enable)
  {
    ...
    gcReceiveSocket = socket(AF_INET, SOCK_DGRAM, 0);
    gcSendSocket = socket(AF_INET, SOCK_DGRAM, 0);
    if(gcReceiveSocket == INVALID_SOCKET || gcSendSocket == INVALID_SOCKET)
      return;
#else
    gcReceiveSocket = socket(AF_INET, SOCK_DGRAM, 0);
    gcSendSocket = socket(AF_INET, SOCK_DGRAM, 0);
    if(gcReceiveSocket < 0 || gcSendSocket < 0)
      return;
    int opt = 1;
    setsockopt(gcReceiveSocket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
#endif

    // Bind receive socket to port 3838
    struct sockaddr_in recvAddr;
    memset(&recvAddr, 0, sizeof(recvAddr));
    recvAddr.sin_family = AF_INET;
    recvAddr.sin_addr.s_addr = INADDR_ANY;
    recvAddr.sin_port = htons(3838);
    if(bind(gcReceiveSocket, reinterpret_cast<struct sockaddr*>(&recvAddr), sizeof(recvAddr)) < 0)
    {
#ifdef WINDOWS
      closesocket(gcReceiveSocket);
      closesocket(gcSendSocket);
      gcReceiveSocket = INVALID_SOCKET;
      gcSendSocket = INVALID_SOCKET;
#else
      close(gcReceiveSocket);
      close(gcSendSocket);
      gcReceiveSocket = -1;
      gcSendSocket = -1;
#endif
    ...
    // Set up send address (broadcast to port 3939)
    memset(&gcSendAddr, 0, sizeof(gcSendAddr));
    gcSendAddr.sin_family = AF_INET;
    gcSendAddr.sin_port = htons(3939);
    gcSendAddr.sin_addr.s_addr = htonl(INADDR_BROADCAST);

    // Enable broadcast on send socket
    int broadcast = 1;
    setsockopt(gcSendSocket, SOL_SOCKET, SO_BROADCAST, reinterpret_cast<const char*>(&broadcast), sizeof(broadcast));

    externalGCEnabled = true;
  }
  else
  {
    externalGCEnabled = false;
#ifdef WINDOWS
    if(gcReceiveSocket != INVALID_SOCKET)
      closesocket(gcReceiveSocket);
    if(gcSendSocket != INVALID_SOCKET)
      closesocket(gcSendSocket);
    gcReceiveSocket = INVALID_SOCKET;
    gcSendSocket = INVALID_SOCKET;
    WSACleanup();
#else
    if(gcReceiveSocket >= 0)
      close(gcReceiveSocket);
    if(gcSendSocket >= 0)
      close(gcSendSocket);
    gcReceiveSocket = -1;
...
{
  if(!externalGCEnabled)
    return;

  // Check for incoming GC data
  fd_set readSet;
  FD_ZERO(&readSet);
#ifdef WINDOWS
  FD_SET(gcReceiveSocket, &readSet);
#else
  FD_SET(gcReceiveSocket, &readSet);
#endif
...
#else
  if(select(gcReceiveSocket + 1, &readSet, nullptr, nullptr, &timeout) > 0)
#endif
  {
    char buffer[4096];
    struct sockaddr_in fromAddr;
    socklen_t fromLen = sizeof(fromAddr);
    ssize_t bytesRead = recvfrom(gcReceiveSocket, buffer, sizeof(buffer), 0,
                              reinterpret_cast<struct sockaddr*>(&fromAddr), &fromLen);
    if(bytesRead > 0)
    {
      // Update gcSendAddr to the address we received from
      gcSendAddr.sin_addr = fromAddr.sin_addr;

      // Parse the GameController return message (RoboCupGameControlData)
      if(bytesRead >= static_cast<ssize_t>(sizeof(RoboCup::RoboCupGameControlData)))
      {
        const RoboCup::RoboCupGameControlData* gcData = reinterpret_cast<const RoboCup::RoboCupGameControlData*>(buffer);
        // Verify the header
        if(gcData->header[0] == 'R' && gcData->header[1] == 'G' && gcData->header[2] == 'm' && gcData->header[3] == 'e')
        {
...
void GameController::sendRobotStatusToGC()
{
  ...
  // Send robot status for each active robot
  for(int i = 0; i < numOfRobots; ++i)
  {
    Robot& r = robots[i];
    if(!r.simulatedRobot || r.info->penalty != PENALTY_NONE)
    ...
    const bool isFirstTeam = i < numOfRobots / 2;
    status.teamNumber = gameControllerData.teams[isFirstTeam ? 0 : 1].teamNumber;
    status.playerNumber = static_cast<uint8_t>(i % MAX_NUM_PLAYERS + 1);
    status.message = 0;

    sendto(gcSendSocket, reinterpret_cast<const char*>(&status), sizeof(status), 0,
           reinterpret_cast<struct sockaddr*>(&gcSendAddr), sizeof(gcSendAddr));
  }
}
