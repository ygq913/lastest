...
void SimulatedRobot::setBallVelocity(const Vector3f& velocity)
{
  if(!ball)
    return;
  Vector3f vel = firstTeam ? (Vector3f() << -velocity.head<2>(), velocity.z()).finished() : velocity;
...
void SimulatedRobot::kickBallPerTeam(const Vector2f& target, float speed)
{
  ...
  Vector2f ballPos = getPosition(ball);
  // Calculate direction from ball to target
  Vector2f direction = (target - ballPos).normalized();
  if(firstTeam)
    direction.x() = -direction.x();
  // Set ball velocity
  Vector3f velocity(direction.x() * speed, direction.y() * speed, 0.f);
  static_cast<SimRobotCore3::Body*>(ball)->setVelocity(velocity.data());
}
