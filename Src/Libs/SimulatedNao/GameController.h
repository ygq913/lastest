...
public:
  ...
  /**
   * Enables or disables the external GameController.
   * @param enable Whether to enable or disable the external GameController.
   */
  void enableExternalGameController(bool enable);

  /**
   * Processes messages from the external GameController.
   */
  void processExternalGameController();

  /**
   * Sends robot status to the external GameController.
   */
  void sendRobotStatusToGC();

  /** Whether the external GameController is enabled. */
  bool externalGCEnabled = false;
...
private:
  ...
  // External GameController UDP sockets
#ifdef WINDOWS
  SOCKET gcReceiveSocket; /**< UDP socket to receive GC messages (port 3838). */
  SOCKET gcSendSocket; /**< UDP socket to send robot status (port 3939). */
#else
  int gcReceiveSocket; /**< UDP socket to receive GC messages (port 3838). */
  int gcSendSocket; /**< UDP socket to send robot status (port 3939). */
#endif
  struct sockaddr_in gcSendAddr; /**< Address of the external GameController. */
};
