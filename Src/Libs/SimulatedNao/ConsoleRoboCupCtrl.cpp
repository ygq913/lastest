...
void ConsoleRoboCupCtrl::update()
{
  ...
  // Process external GameController
  if(gameController.externalGCEnabled)
  {
    gameController.processExternalGameController();
    gameController.sendRobotStatusToGC();
  }

  // Process commands from external CommandServer
  std::vector<std::string> externalCommands = commandServer.getPendingCommands();
  for(const std::string& cmd : externalCommands)
  {
...
bool ConsoleRoboCupCtrl::executeConsoleCommand(std::string command, RobotConsole* console)
{
  ...
  else if(buffer == "gc")
  {
    ...
    else if(buffer == "external")
    {
      stream >> buffer;
      if(buffer == "on")
        gameController.enableExternalGameController(true);
      else if(buffer == "off")
        gameController.enableExternalGameController(false);
      else
        printLn("Syntax Error: gc external on|off");
    }
    else
      printLn("Syntax Error");
  }
